...
def print_ComplexType(out, c):
    ...
    if c.children is not None:
        print_children(out, c.children)

    attrs = c.attrs

    if len(attrs) > 0:
        if c.children is not None: out.nl()
        for a in attrs:
            print_Attribute(out, a)

    if c.any_attr is not None:
        out.nl()
        print_AnyAttribute(out, c.any_attr)

    out.dec()
    out.write("}").nl()


def print_children(out, c):
    assert isinstance(c, P_Children)
    v = Children_Visitor(out)
    c.visit(v)


class Children_Visitor (object):
    ...
    def visit_Seq(self, c):
        assert isinstance(c, P_Seq)
        print_Seq(self.out, c)

    def visit_LocalAny(self, c):
        assert isinstance(c, P_LocalAny)
        print_LocalAny(self.out, c)

    def visit_GenExpr(self, c):
        assert isinstance(c, P_GenExpr)
        print_GenExpr(self.out, c)
...
class Expr_Visitor (object):
    ...
    def visit_Expr_Seq(self, c):
        assert isinstance(c, P_Expr_Seq)
        print_expr_seq(self.out, c)
...
def print_expr_choid(out, c):
    assert isinstance(c, P_Expr_Choice)
    out.write("Choice {").nl()
    out.inc()
